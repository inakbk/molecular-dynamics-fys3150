...
In figure \ref{fig: D_T} we see the temperature ratio $T_{eq}/T_i$ (top) and the diffusion constant (bottom) after reaching equilibrium $D_{eq}$ plotted versus the instant temperature $T_{eq}$ where$T_{eq}$ is the instant temperature after 10000 timesteps. We see that the diffusion constant is zero for all temperatures $T_{eq}$ up to a point where it suddenly grows. It apeares as if the gowth is quite linear, but there is a lot of noice in the datapoints. We can also see that the lines cross at one point for high temperatures. In the top plot

Clearly somerthing is happening at $T=2.3$....but also a lot of noise plobably from that unit cells is 5
